...
class DBHelper {
    ...
    static func applicationDocumentsDirectoryFile(_ fileName: String) -> [CChar]? {
        ...
        let path = (documentDirectory[0] as NSString).appendingPathComponent(DB_FILE_NAME)
    ...
    static func initDB() {
        ...
        if dbConfigVersion?.int32Value != versionNubmer {
            ...
            if sqlite3_open(dbFilePath!, &db) == SQLITE_OK {
                ...
                //把当前版本号写回到文件中
                let usql = String(format: "update  DBVersionInfo set version_number = %i", dbConfigVersion!.intValue)
                let cusql = usql.cString(using: String.Encoding.utf8)
    ...
    private static func dbVersionNubmer() -> Int32 {
