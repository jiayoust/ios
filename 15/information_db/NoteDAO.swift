...
class NoteDAO {
    ...
    private func createEditableCopyOfDatabaseIfNeeded() {
        ...
        if sqlite3_open(cpath!, &db) != SQLITE_OK {
            ...
            let sql = "CREATE TABLE IF NOT EXISTS Note (cname TEXT PRIMARY KEY, ctelno TEXT)"
    ...
    func create(_ model: Note) -> Int {
        ...
        if sqlite3_open(cpath!, &db) != SQLITE_OK {
            ...
            let sql = "INSERT OR REPLACE INTO note (cname, ctelno) VALUES (?,?)"
            let cSql = sql.cString(using: String.Encoding.utf8)
            ...
            if sqlite3_prepare_v2(db, cSql!, -1, &statement, nil) == SQLITE_OK {
                ...
                let ctelno = model.telno.cString(using: String.Encoding.utf8)
                
                //绑定参数开始
                sqlite3_bind_text(statement, 1, cname!, -1, nil)
                sqlite3_bind_text(statement, 2, ctelno!, -1, nil)
    ...
    func modify(_ model: Note) -> Int {
        ...
        if sqlite3_open(cpath!, &db) != SQLITE_OK {
            ...
            let sql = "UPDATE note set ctelno=? where cname =?"
            let cSql = sql.cString(using: String.Encoding.utf8)
            ...
            if sqlite3_prepare_v2(db, cSql!, -1, &statement, nil) == SQLITE_OK {
                ...
                //绑定参数开始
                sqlite3_bind_text(statement, 1, ctelno!, -1, nil)
    ...
    func findAll() -> [Note] {
        ...
        if sqlite3_open(cpath!, &db) != SQLITE_OK {
            ...
            let sql = "SELECT cname,ctelno FROM Note"
            let cSql = sql.cString(using: String.Encoding.utf8)
            ...
            if sqlite3_prepare_v2(db, cSql!, -1, &statement, nil) == SQLITE_OK {
                ...
                while sqlite3_step(statement) == SQLITE_ROW {
                    ...
                    if let telno = getColumnValue(index:1, stmt:statement!) {
                        note.telno = telno
                    }
    ...
    func findById(_ model: Note) -> Note? {
        ...
        if sqlite3_open(cpath!, &db) != SQLITE_OK {
            ...
            let sql = "SELECT cname,ctelno FROM Note where cname =?"
            let cSql = sql.cString(using: String.Encoding.utf8)
